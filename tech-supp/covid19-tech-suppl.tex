...
\chapter{Overview}\label{chap:tech-supp-overview}
...
All infection site work in the same essential way. 
A person is \defi{infectious} if the current day is between the date of infection and the
end of the person's incubation period. People whose incubation period has passed; 
during the illness period, individuals do not infect others.
...
\onecolumn
\section{Default epidemiological}
...
Incubation mean & 5 days \\
Illness distribution & Poisson \\ 
Illness mean & 15 days \\
\code{a\_workplace\_rate} & 7\% & Probability that a workplace can support reinfection \\
\code{a\_household\_rate} & 15\% & \\ %Probability that a household can support reinfection \\
\code{a\_schools\_rate} & 15\% & \\ %Probability that a school can support reinfection \\ 
...
\code{q\_household} & 1/20 \\
\code{q\_school} & 1/3000 & \\
\code{q\_supermarket} & 1/1000 & \\
\code{q\_places} & 1/500 & \\
...
\code{p\_asympto} & 48\% & The proportion of cases that do not develop symptoms \\
\code{p\_critical} & 2\% & The proportion of symptomatic cases that become critical.\\
\code{p\_death} & 1\% & The proportion of critical cases that die. \\
